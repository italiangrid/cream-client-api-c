...
CreamProxy_Cancel::CreamProxy_Cancel( const JobFilterWrapper* filter, 
				      ResultWrapper* target, const int timeout ) 
  throw() : AbsCreamProxy( timeout ), m_jobFilter( filter ), m_result( target )
{
}

//______________________________________________________________________________
void CreamProxy_Cancel::execute( const string& serviceAddress, const bool ignore_tag_mismatch ) 
  throw(BaseException&,
...
#ifdef DOPROFILE
    apiUtil::scoped_timer T("CreamProxy_Cancel::execute-soap_call___CREAM__JobCancel");
#endif  
    
    
    
    if(SOAP_OK != soap_call___CREAM__JobCancel(m_soap,
